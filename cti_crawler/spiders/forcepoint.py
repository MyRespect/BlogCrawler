...
web_name='forcepoint_blog'
web_address='https://www.forcepoint.com'

class CybersecurityAttSpider(scrapy.Spider):
    name = 'forcepoint_blog'
    # allowed_domains = ['forcepoint.com']
    ...
    def parse(self, response):
        ...
        blog_urls=response.xpath("//div[@class='blog__info']/h2/a/@href").extract()
    ...
    def parse_blog(self, response):
        ...
        item['title'] = escape_string(''.join(response.xpath("//div[@class='field-title-field']/h1/text()").extract()).strip()) 
        item['publish_date'] = escape_string(''.join(response.xpath("//div[@class='panel-pane pane-node-created']/div[@class='pane-content']/text()").extract()))
        item['author'] = escape_string(''.join(response.xpath("//div[@class='profile__info']/span[@class='profile__full-name']/a/text()").extract()))
        item['tags'] = escape_string(''.join(response.xpath("//div[@class='panel-pane pane-entity-field pane-node-field-tags']/div[@class='pane-content']/a[/*]/text()").extract()))
        item['contents'] = escape_string(' '.join(response.xpath("//div[@class='content']/div[@class='field-pg-wysiwyg-body']//*/text()").extract()).strip())
