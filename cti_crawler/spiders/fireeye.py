...
web_name='fireeye_blog'
web_address='https://www.fireeye.com'

class CybersecurityAttSpider(scrapy.Spider):
    name = 'fireeye_blog'
    # allowed_domains = ['fireeye.com']
    ...
    def parse(self, response):
        ...
        blog_urls=response.xpath("//div/ul[@class='blog-tiles-v2']/div[@class='c11v9'][*]/a/@href").extract()
    ...
    def parse_blog(self, response):
        ...
        item['title'] = escape_string(''.join(response.xpath("//div[@class='title entrytitle']/h1[@class='blogpostlanding red']/text()").extract()).strip()) 
        item['publish_date'] = escape_string(''.join(response.xpath("//div[@class='title entrytitle']/div[@class='c00 c00v0 entry-meta-block selfClear']/time[@class='entry-date']/text()").extract()))
        item['author'] = escape_string(''.join(response.xpath("//div[@class='title entrytitle']/div[@class='c00 c00v0 entry-meta-block selfClear']/span[@class='by-author']/a/text()").extract()))
        item['tags'] = escape_string(''.join(response.xpath("//div[@class='tags-section']/a[*]/span[@class='tagbutton']/text()").extract()).strip()) 
        item['contents'] = escape_string(' '.join(response.xpath("//div[@class='c00 c00v0']//*/text()").extract()).strip())
