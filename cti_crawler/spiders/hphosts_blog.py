...
web_name='hphosts_blog'
web_address='https://hphosts.blogspot.com/'

class CybersecurityAttSpider(scrapy.Spider):
    name = 'hphosts_blog'
    # allowed_domains = ['blogspot.com']
    start_urls = ['https://hphosts.blogspot.com/']
    ...
    def parse_blog(self, response):
        ...
        item['title'] = escape_string(' '.join(response.xpath("//h3[@class='post-title entry-title']/a/text()").extract()).strip())
        item['publish_date'] = escape_string(' '.join(response.xpath("///h2[@class='date-header']/span/text()").extract()).strip())
        item['author'] = "none"
        item['tags'] = "none"
        item['contents'] = escape_string(' '.join(response.xpath("//div[@class='post-body entry-content']/text()").extract()).strip())
